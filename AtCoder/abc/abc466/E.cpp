...
int main() {
    ...
    int N, K;
    cin >> N >> K;
    auto AB = make_v<int>(N, 2);
    FOR(i, 0, N) {
        cin >> AB[i][0] >> AB[i][1];
    }

    auto dp = make_v<LL>(N , 2, K + 1);
    fill_v(dp, -LINF);
    dp[0][0][K] = AB[0][0];
    dp[0][1][K - 1] = AB[0][1];
    ...
    FOR(i, 0, N - 1) {
        FOR(j, 0, 2) {
            FOR(k, 0, K + 1) {
                ...
                // 右にいく
                chmax(dp[i + 1][j][k], now + AB[i + 1][j]);

                // 右下にいく
                if (j == 0 && k > 0) {
                    chmax(dp[i + 1][1][k - 1], now + AB[i + 1][j + 1]);
                }

                // 右上にいく
                if (j == 1) {
                    chmax(dp[i + 1][0][k], now + AB[i + 1][j - 1]);
                }
            }
        }
    }
    ...
    LL ans = 0;
    FOR(k, 0, K + 1) {
        chmax(ans, dp[N - 1][0][k]);
        chmax(ans, dp[N - 1][1][k]);
    }
    print(ans);
